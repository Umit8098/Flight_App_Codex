...
## Monorepo Run (Current Structure)

This project now runs in a monorepo layout:

- `Flight_App_CH-12_Codex/backend` -> Django API
- `Flight_App_CH-12_Codex/frontend` -> React UI
...
- `/` -> Flights home list
- `/flights/:id` -> Flight detail + reservation management
- `/my-reservations` -> User reservation list (auth required)
- `/profile` -> User info + password change (auth required)
- `/staff/flights` -> Staff flight CRUD (staff only)
- `/login`, `/register`

## Quick Smoke Test

Use [`SMOKE_TEST_CHECKLIST.md`](../SMOKE_TEST_CHECKLIST.md) for end-to-end manual verification.
